    ...
    def run(self):  # method which runs the thread
        ...
        while (not self.done): # loop for getting data from actuator
            ...
            if out_error[:-2] != '0000':     # each TS call, erase the previous error
                self.emit(SIGNAL("error_send(QString)"),out_error[:-2]) # send to main thread to be displayed the last error
    ...
    def run(self):  # method which runs the thread
        ...
        while (not self.done_i_direction): # loop for sweeping current
            ...
            if (self.go_to_origin == True):    # move bead to origin section activate?
                ...
                cond =((abs(self.x_bead-x_mid)<10) and (abs(self.y_bead-y_mid)<10) and (abs(self.z_bead-z_mid)<0.010))
                #print 'cond', cond
                ...
                elif (self.z_bead <= z_mid+0.01) and (self.go_to_origin_go ==True):  # bead is not in the center and bellow z_center
                    iii=3  # we go forward
            ...
            if (self.go_away_from_origin ==True):  # apply one of the configurations
                ...
                cond2 =((abs(self.x_bead-x_mid)>300) or (abs(self.y_bead-y_mid)>300) or (abs(self.z_bead-z_mid)>0.11))
                #print 'cond2', cond2

                #if ((cond2 == True) or ((time.clock() - time_old) > 15)): # intitial condition for time
                if ((cond2 == True) or ((time.clock() - time_old) > 600)):  # here we don't care about time. what all traj
                    self.emit(SIGNAL("I_exp_ZERO()")) # set current to zero
                    self.go_away_from_origin =False
                    self.go_away_from_origin_1st =True
                    self.go_to_origin = True
...
#atexit.register(camcapture_2.runFeatureCommand("AcquisitionStop"))
    ...
    def initUI(self):
        ...
        self.connect(self.butt_plots_tab_letter_seq_tlaps_start,SIGNAL("pressed()"),self.butt_plots_tab_letter_seq_tlaps_start_Changed)
        ...
        self.connect(self.butt_plots_tab_letter_seq_tlaps_stop,SIGNAL("pressed()"),self.butt_plots_tab_letter_seq_tlaps_stop_Changed)
        ...
        # &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
        self.butt_act_start =QtGui.QPushButton("Start pooling actuator",self)
        self.connect(self.butt_act_start,SIGNAL("pressed()"),self.butt_act_start_Changed)

                # ==============================================

        self.butt_act_stop =QtGui.QPushButton("Stop pooling actuator",self)
        ...
        # &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
        self.butt_ard_start =QtGui.QPushButton("Start pooling arduino",self)
        self.connect(self.butt_ard_start,SIGNAL("pressed()"),self.butt_ard_start_Changed)

                # ==============================================

        self.butt_ard_stop =QtGui.QPushButton("Stop pooling arduino",self)
        ...
        self.connect(self.th_actuator,SIGNAL("error_status_send(QString)"),self.error_status_display,Qt.DirectConnection)
        ...
        self.connect(self.th_actuator,SIGNAL("adj_manual_z_for_tracking()"),self.error_lim_disable_track,Qt.DirectConnection)
        ...
        self.connect(self.th_power_supp,SIGNAL("temp_C_send3(QString)"),self.temperature_display3,Qt.DirectConnection)
        ...
        self.connect(self.th_experiments,SIGNAL("experiment_save_picture()"),self.update_save_pict,Qt.DirectConnection)
        self.connect(self.th_experiments,SIGNAL("log_exp_stack_init()"),self.log_exp_stack_init,Qt.DirectConnection)
        self.connect(self.th_experiments,SIGNAL("log_exp_stack_running()"),self.log_exp_stack_running,Qt.DirectConnection)

        self.connect(self.th_exp_dir,SIGNAL("I_exp_actual_index_send(QString)"), self.update_index_exp_i_dir, Qt.DirectConnection)
...
#        self.th_temper_2.start()
    ...
    def butt_start_rec_fileout_Changed_new(self):
        ...
        to_write_2 = to_write_2 + ' spim_box_small(Y/N), spim_box_big(Y/N) ' +str(self.z_bead_bottom)#+' , temp3(C)#, temp4(C)'
        ...
        to_write = to_write + 'temp(C), temp2(C), temp3(C), I1(A), I2(A), I3(A), I4(A),PWS(ON/OFF) ' +str(self.z_bead_bottom)
        ...
        if ((abs(xxx_spim_air_2)<=(226.0+50.0)) and (abs(yyy_spim_air_2)<=226.0+50.0) and (abs(zzz_spim_air_2)<=(50.0+40.0))):
            spim_box_2 = 'YES'
        else:
            spim_box_2='NO'
    ...
    def butt_save_fileout_Changed(self):
        ...
        for ii in range(0,2):
            to_write = str(self.cal_t[ii])+','+str(self.cal_t[ii])+','+str(self.cal_z[ii])+','+str(self.cal_xc1[ii])+','
            to_write = to_write             +str(self.cal_yc1[ii])+','+str(self.cal_foc1[ii])+','
            to_write = to_write             +str(self.cal_xc2[ii])+','+str(self.cal_yc2[ii])+','
            to_write = to_write             +str(self.cal_foc2[ii])+','+str(self.cal_temp[ii])+','+str(self.cal_temp2[ii])+','
        ...
        for ii in range(2,len(self.cal_t)):
            ...
            to_write = str(self.cal_t[ii])+','+str(ttime_corect_new)+','+str(self.cal_z[ii])+','+str(self.cal_xc1[ii])+','
            to_write = to_write             +str(self.cal_yc1[ii])+','+str(self.cal_foc1[ii])+','
            to_write = to_write             +str(self.cal_xc2[ii])+','+str(self.cal_yc2[ii])+','
    ...
    def log_data_time_laps(self):
        path_file = os.getcwd()
        path_file = path_file + '\\time_laps\\'+'log_time_laps.dat'
        f_name = open(path_file, 'a')
...
#        return
    ...
    def drawFrames(self, qp):
        ...
        x81 = 1000; x82 = x81 + 100
        y81 = 710;  y82 = y81 + 220

        x91 = 780;   x92 = x91 + 80
        y91 = 1070;  y92 = y91 + 180

        x101 = 1490;   x102 = x101 + 100
        y101 = 550;    y102 = y101 + 520

        x111 = 1740;   x112 = x111 + 130
        y111 = 970;  y112 = y111 + 250

        x121 = 1380;   x122 = x121 + 120
        y121 = 170;  y122 = y121 + 1040

        x131 = 1630;   x132 = x131 + 160
        y131 = 450;  y132 = y131 + 270

        x141 = 1000;   x142 = x141 + 90
        y141 = 720;  y142 = y141 + 310
        ...
        if QtGui.QAbstractButton.isChecked(self.checkbox_XZ_section):
            ...
            cv2.line(n_frame_xz,(1,130+delta_z_center_lim_mmax_xz),(259,130+delta_z_center_lim_mmax_xz),(0,0,0),1)
        ...
        if QtGui.QAbstractButton.isChecked(self.checkbox_b): # Detect beads is selected
            ...
            if no_beads > 0: # At least one bead is detected
                ...
                if QtGui.QAbstractButton.isEnabled(self.butt_ard_stop): # PWS can be controlled
                    ...
                    if QtGui.QAbstractButton.isChecked(self.checkbox_center) and (time.clock()-self.time_old_ard)>0.5:
                    ...
                    if QtGui.QAbstractButton.isChecked(self.checkbox_write) and (time.clock()-self.time_old_ard)>0.5:
                        if ((xc >x_min and xc<x_max) and (yc >y_min and yc<y_max)):
